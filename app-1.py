...
# 캐시된 데이터 로드
# ══════════════════════════════════════════════
@st.cache_data(ttl=3600)   # 1시간마다 새로 받아옴
def load_data(start):
...
def backtest(price_curve, signal_df, bull_col, bear_col,
    ...
    RSI 선제투입 규칙
    ─────────────────
    • 일봉 RSI < rsi_prebuy_thresh 이면서 현금이 있을 때 선제매수
    • 사이클당 1회만 실행 (prebuy_done 플래그)
    • Bear 신호가 확인되면 사이클 초기화 (prebuy_done = False)
    • Bull 신호 매수와 중복 실행 방지 (같은 날 bull 이면 bull 우선)
    """
    ...
    # 선제투입 사이클 플래그
    prebuy_done = False   # 현재 사이클에서 선제투입을 이미 했는지
    ...
    for i in range(1, n):
        ...
        # ── ❶ Bull 신호: 현금 전액 투입 (사이클 초기화 없음 — bull은 공격적 매수)
        ...
        # ── ❷ Bear 신호: 일부 매도 + 사이클 초기화
            ...
            # Bear 신호 → 사이클 초기화 (다음 선제투입 허용)
            prebuy_done = False

        # ── ❸ RSI 선제투입: bull/bear 없고, 아직 미실행, 현금 있을 때
        elif (use_rsi_prebuy
              and not prebuy_done
              and not prev_bull
              and not prev_bear
              and cash[i] > 1e-6):
            rsi_val = daily_rsi.iloc[i-1]   # 전날 RSI 기준 (당일 시가 매수)
            if pd.notna(rsi_val) and rsi_val < rsi_prebuy_thresh:
                ...
                if invest_cash > 1e-6:
                    ...
                    prebuy_done = True   # 이 사이클에서 선제투입 완료
...
def run_all(start, rsi_period, left, right, min_range, max_range,
    ...
    # 일봉 RSI 계산해서 df에 추가 (탭2 차트용)
    df["daily_rsi"] = calc_rsi(df["Close"], rsi_period)
    # 선제투입 발생 시점 표시용 컬럼 (시각화용, 실제 backtest에서 내부 계산)
    # backtest 실행 후 trade_rows에서 추출

    res_div_1x, tr_div_1x = backtest(df["px_1x"], df, "bull_event","bear_event",
        initial_capital, bear_sell_pct, bear_sell_basis, _apply_tax,
        use_rsi_prebuy, rsi_prebuy_thresh, rsi_prebuy_pct)
    ...
    res_sma_1x, tr_sma_1x = backtest_sma200(df["px_1x"], df, initial_capital, _apply_tax)
    res_sma_2x, tr_sma_2x = backtest_sma200(df["px_2x"], df, initial_capital, _apply_tax)
    res_sma_3x, tr_sma_3x = backtest_sma200(df["px_3x"], df, initial_capital, _apply_tax)
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
    "📊 전략 비교", "📉 신호 차트", "🗓️ 10년 단위 분석", "📋 매매 기록", "📈 비중 & MDD", "🔬 전진분석 (WFA)"
])
...
with tab1:
    ...
    bh_eq, div_eq, sma_eq = lev_map[lev_choice]

    # 지표 카드
    summary_rows = [
        calc_stats(bh_eq,  f"BH {lev_choice}"),
        calc_stats(div_eq, f"Div {lev_choice} (세후)"),
        calc_stats(sma_eq, f"SMA200 {lev_choice} (세후)"),
    ]
    cols = st.columns(3)
    ...
    st.subheader("성과 요약표")
    all_stats = [
        calc_stats(bh_1x,              "BH 1x"),
        calc_stats(res_div_1x["equity"],"Div 1x (세후)"),
        calc_stats(res_sma_1x["equity"],"SMA200 1x (세후)"),
        calc_stats(bh_2x,              "BH 2x"),
        calc_stats(res_div_2x["equity"],"Div 2x (세후)"),
        calc_stats(res_sma_2x["equity"],"SMA200 2x (세후)"),
        calc_stats(bh_3x,              "BH 3x"),
        calc_stats(res_div_3x["equity"],"Div 3x (세후)"),
        calc_stats(res_sma_3x["equity"],"SMA200 3x (세후)"),
    ]
...
with tab2:
    ...
    # RSI 선제투입 시점 추출 (1x 기준 trade 기록 활용)
...
# ──────────────────────────────────────────────
# 탭 6: Walk-Forward Analysis (전진분석)
# ──────────────────────────────────────────────

@st.cache_data(ttl=3600, show_spinner=False)
def run_wfa(start, is_years, oos_years,
            expense, spread_2x, spread_3x, cost_one_way,
            bear_sell_pct, bear_sell_basis, initial_capital, _apply_tax,
...
with tab6:
    ...
    if "wfa_df" in st.session_state and len(st.session_state["wfa_df"]):
        ...
        fig_wfa1.update_layout(
            title="창별 IS vs OOS CAGR 비교 (IS가 높고 OOS가 따라올수록 좋음)",
        ...
        fig_wfa2.update_layout(
            title="효율비율 (OOS CAGR ÷ IS CAGR) — 0.5 이상이면 과최적화 위험 낮음",
        ...
        st.caption("구간마다 다른 파라미터가 최적이면 전략이 불안정한 것입니다. 같은 값이 반복될수록 좋습니다.")
...
st.divider()
st.caption(f"데이터: Yahoo Finance (^NDX) | 마지막 갱신: {df.index[-1].date()} | "
