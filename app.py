...
# ══════════════════════════════════════════════
# 메인 실행
# ★ 핵심 수정: apply_tax_flag (언더스코어 제거 → 캐시 키에 포함됨)
# ══════════════════════════════════════════════
...
def run_all(start, rsi_period, left, right, min_range, max_range,
            ...
            initial_capital, apply_tax_flag,          # ★ 변경
    ...
    # ── 세전 결과 (apply_tax_flag 와 무관하게 항상 계산)
    ...
    res_sma_1x_pre, _ = backtest_sma200(df["px_1x"], df, initial_capital, False)
    res_sma_2x_pre, _ = backtest_sma200(df["px_2x"], df, initial_capital, False)
    res_sma_3x_pre, _ = backtest_sma200(df["px_3x"], df, initial_capital, False)
...
with st.spinner("데이터 다운로드 & 백테스트 계산 중..."):
    (df, bull_table, bear_table,
     ...
     tr_sma_1x, tr_sma_2x, tr_sma_3x,
     res_div_1x_pre, res_div_2x_pre, res_div_3x_pre,
     res_sma_1x_pre, res_sma_2x_pre, res_sma_3x_pre) = run_all(
        START, RSI_PERIOD, LEFT, RIGHT, MIN_RANGE, MAX_RANGE,
        EXPENSE, SPREAD_2X, SPREAD_3X, COST_ONE_WAY,
        SMA_PERIOD, BEAR_SELL_PCT, BEAR_SELL_BASIS,
        INITIAL_CAPITAL, apply_tax,                   # ★ apply_tax_flag 로 전달
        TAX_RATE, TAX_DEDUCTION,
        USE_RSI_PREBUY, RSI_PREBUY_THRESH, RSI_PREBUY_PCT)
...
with tab1:
    ...
    summary_rows = [
        ...
        calc_stats(div_eq, f"Div {lev_choice} ({tax_label})"),
    ...
    # 세전/세후 차이(Δ) 열 추가
    def delta_row(pre_row, post_row):
        """세전→세후 변화를 숫자로 계산해 문자열로 반환"""
        def pct_val(s):
            return float(s.replace("%","").replace("+",""))
        def usd_val(s):
            return float(s.replace("$","").replace(",",""))
        cagr_d  = pct_val(post_row["CAGR"])  - pct_val(pre_row["CAGR"])
        final_d = usd_val(post_row["최종자산"]) - usd_val(pre_row["최종자산"])
        return f"CAGR {cagr_d:+.2f}%p  /  최종자산 ${final_d:+,.0f}"
    ...
    st.dataframe(
        stats_df.style.apply(highlight_rows, axis=1),
    ...
    st.dataframe(
        delta_df.style.apply(highlight_delta, axis=1),
        use_container_width=True,
        hide_index=True
    )
...
def run_wfa(start, is_years, oos_years,
            expense, spread_2x, spread_3x, cost_one_way,
            bear_sell_pct, bear_sell_basis, initial_capital, apply_tax_flag,  # ★ 변경
